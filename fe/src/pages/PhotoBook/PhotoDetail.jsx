...
const PhotoDetail = () => {
  ...
  const DeletePhoto = () => {
    ...
    httpClient.delete("/albums/album/" + reqData)
    .then((data) => {
      console.log(data)
      alert("album deleted");
      navigate("/photobook");
  ...
  return (
    <Box>
      ...
      </IconButton>
      {
        loaded == true &&
        <>
          <Button onClick={DeletePhoto} variant='outlined'>사진 삭제</Button>
          <PhotoCard photo={photoDetail} style={{ width: 'auto' }} variant="detail" />
          <CommentInput2 photoId={photoDetail.id} />
        </>
      }
      {
        photoDetail != null && photoDetail.albumCommentDtos != null &&
