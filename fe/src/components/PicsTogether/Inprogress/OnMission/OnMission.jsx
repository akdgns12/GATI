...
export default function OnMission() {
  ...
  if (getMission) {
    if (getMission.missionImageDtos) {
      getMission.missionImageDtos.map((dto, index) =>
        content.push(<ShowPics key={index + 15} dto={dto} />)
      );
  ...
  return (
    <Box>
      ...
      >
        ...
        {onDesc === true ? (
          <Typography margin="20px" fontSize="13px">
            {" "}
            {getMission.content}
          </Typography>
        ) : null}
        <Typography color="#888888">
          {" "}
          {getMission.startDate} ~ {getMission.endDate}{" "}
        </Typography>
      </Box>
